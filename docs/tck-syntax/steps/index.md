...
| `cac` | O **[PROP]** | CAC identifiers (`<standard-id>:<standard-version>:<cac-id>`) this step verifies. Replaces the test-level `cac` for this step. Also accepted on each `validate[]` entry, where it replaces the step's. Experimental extension: needs `extensions: [cac]` in `index.yaml`, and the standard must be in `metadata.standards`. See [§9.1](../extensions.md#91-cac-traceability-cac-p1). |
