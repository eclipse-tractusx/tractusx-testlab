...
class TestDescriptorSerialisation:
    ...
    def test_the_script_spelling_is_accepted_on_the_way_in(self) -> None:
        value = {"id": _SHELL_ID, "id_short": "twin-a", "specificAssetIds": []}
        assert DescriptorPayload.of(value).id_short == "twin-a"
...
class TestShellLookup:
    ...
    async def test_the_lookup_is_addressed_to_the_registry_behind_the_dataplane(
        ...
        url, kwargs = get.call_args.args[1], get.call_args.kwargs
    ...
    async def test_the_dataplane_pair_falls_back_to_what_the_transfer_published(
        ...
        assert get.call_args.args[1] == f"{_DATAPLANE}/lookup/shells"
    ...
    async def test_each_descriptor_is_read_by_its_encoded_identifier(
        ...
        descriptor_url = get.call_args_list[1].args[1]
...
def registry_context(context: MagicMock) -> MagicMock:
    ...
    context.dataspace.registry = MagicMock(return_value=aas)
    return context
...
class TestShellLookupByAssetLink:
    ...
    async def test_the_dataplane_pair_falls_back_to_what_the_transfer_published(
        ...
        assert post.call_args.args[1] == f"{_DATAPLANE}/lookup/shellsByAssetLink"
...
class TestDataplaneGetShellDescriptor:
    ...
    async def test_reads_the_descriptor_by_its_encoded_identifier(self, context: MagicMock) -> None:
        ...
        encoded = get.call_args.args[1].rsplit("/", 1)[-1]
        assert base64.urlsafe_b64decode(encoded + "==").decode() == _SHELL_ID
        assert get.call_args.kwargs["headers"]["Authorization"] == _TOKEN
