...
from tractusx_testlab.steps.connector.dataplane import edr_entry_of
...
async def _do_dsp_flow(
    ...
    except (httpx.HTTPError, ConnectionError, TimeoutError, StepExecutionError) as exc:
        # StepExecutionError included: a pre-fetch that ran out of time is still
        # only a pre-fetch. The flow below queries the same catalog under its own
        # budget, and that is the call whose failure is the step's.
    ...
    edr_entry = await edr_entry_of(consumer, transfer_id)
