...
from tractusx_testlab.steps.assertions.operands import (
    Check,
    OperandError,
    bounded,
    has_key,
...
TABLE: tuple[Operator, ...] = (
    ...
    Operator(
        "not_equals",
        Arity.BINARY,
        lambda actual, expected: not same(actual, expected),
    ...
    Operator(
        "none_of",
        Arity.BINARY,
        lambda actual, expected: not is_member(actual, expected),
    ...
    Operator(
        "not_has_key",
        Arity.BINARY,
        lambda actual, expected: not has_key(actual, expected),
        "Expected {actual!r} to NOT have key {expected!r}",
    ),
    Operator(
        "gt",
        Arity.BINARY,
        numeric(lambda left, right: left > right),
    ...
    Operator(
        "length_equals",
        Arity.BINARY,
        sized(lambda length, wanted: length == wanted),
    ...
    Operator(
        "between",
        Arity.RANGE,
        bounded(lambda value, low, high: low <= value <= high),
