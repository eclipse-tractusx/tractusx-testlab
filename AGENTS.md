...
| AI Disclosure block embedded in the license header | See below |
| Variable syntax in YAML: `${{ ... }}` (e.g. `${{ env.X }}`, `${{ steps.<id>.<out> }}`) per ADR-0010 | Never `@variable_name` or `${var}` |

### License Header & AI Disclosure

All AI-generated or AI-modified files must carry the Eclipse Tractus-X license
header with the **AI Disclosure block embedded**. The disclosure records how much
of the file is AI-generated (`minimally | largely | entirely`), the assisting
provider/model/version, that AI portions are released under **CC0-1.0**, and that
...
The `Assisted-by` line follows the pattern `Assisted-by: [Provider] [Model-Family] ([Version/ID])`
(e.g. `Assisted-by: Google gemini-2.5-pro (rev-1)`). Use `//` markers for
Java/C/TS/Go/Rust and `<!-- -->` for HTML/XML — only the comment markers change,
never the wording. See
`.github/instructions/ai_generated_code.instructions.md` for the full
language variants and rules.
